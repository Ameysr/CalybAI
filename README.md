...
|-------|-------------|---------------------------|
| **Crawl** | Searches OpenAlex for topic papers up to 50 seeds. Collects referenced works ranked by how many seeds cite them. Fetches citing papers. Batch resolves placeholder references. | Keyword overlap relevance filter excludes irrelevant papers. target times 2 cap prevents unbounded growth. Batch fetch uses openalex multi ID filter 50 per call 0.05s delay. |
...
| **Analyze** | Computes PageRank alpha 0.85, in degree, out degree, betweenness centrality k sampled at 100 for large graphs. Runs Louvain community detection. | All metric results cached via lazy init in Analyzer class. Betweenness skips graphs over 500 nodes. Falls back to greedy modularity if python louvain not installed. |
| **Order** | Computes citation depth longest path from root. Sorts by depth to year ascending to PageRank descending. Handles cycles via SCC condensation when topological sort fails. | Curriculum receives precomputed metrics from pipeline, never re runs pagerank. Cycle resolution uses strongly connected components plus condensed DAG. |
| **Export** | Returns reading order as ranked list with metadata. CSV export via api export.csv. JSON export via api export.json. Graph JSON with nodes and edges for frontend. | CSV generated server side with proper quoting. JSON export returns full analysis results. Frontend uses anchor download for file downloads. |
...
Open http://127.0.0.1:8000 in your browser, enter a research topic, and click Run Analysis. The pipeline will crawl OpenAlex for 25+ papers on your topic, build the citation graph, compute influence metrics, and display the interactive graph, reading curriculum and statistics.
...
Outputs the same analysis directly to the terminal including reading order, foundational papers, surveys and graph statistics.

## API Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | /api/run | Run the full pipeline. Body: topic and target. Runs in background thread via run_in_executor. |
...
| GET | /api/foundational | Return top foundational papers by combined PageRank plus rank score. |
...
| Frontend | Vanilla JS plus vis network 9.1 interactive graph |
